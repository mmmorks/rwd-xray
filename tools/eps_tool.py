...
def main():
    ...
    with open(os.path.join(sys.path[0], "user.bin"), "rb+") as f:
        ...
        elif len(input_bin) == 327680:
            data_start = 0x0
            data_size = 0x50000
            checksum_offsets = [(0, 0x12fd8), (1, 0x12fd4)] #original bin checksums are at offsets 0x6FF80 and 0x6FFFE, but since we start the bin from 0x10000 after bootloader, we offset the checksum accordingly
        ...
        elif input_bin_hash == '4f3b6c8242d198d08ae357a7d232669f': # mmmorks
            print('Detected bin: 39990-TG7-A030 Honda Pilot')
            supported_versions = ['39990-TG7-A020\x00\x00', '39990-TG7-A030\x00\x00']
            security_key = ['\x00\x11\x00\x12\x10\x20', '\x00\x11\x00\x12\x10\x20']
            version_offsets = [0x3438]
            version_old = b'39990-TG7-A030'
            version_new = b'39990-TG7,A030'
            data_offsets = [
                0x11964, #speed_clamp_lo
...
## do patch
    if args.stock:
        ...
        with open(os.path.join(sys.path[0], "user.bin"), 'rb+') as f:
            full_fw = f.read()
            patch_fw = full_fw[data_start:(data_start + data_size)]
    else:
...
## build rwd
    start_addr = param_to_data_string(hex(data_start))
