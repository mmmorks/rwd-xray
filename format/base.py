...
class Base(object):
    ...
    def decrypt(self, search_value):
        search_value_bytes = search_value.encode('ascii')
        search_value_padded = ''.join(c + '.' for c in search_value)
        search_value_padded_bytes = search_value_padded.encode('ascii')
        print("search:")
        print(search_value)
        print(search_value_padded)

        search_exact = re.compile(b'.*'+search_value_bytes+b'.*', flags=re.IGNORECASE|re.MULTILINE|re.DOTALL)
        # sometimes there is an extra character after each character
        # 37805-RBB-J530 -> 3377880550--RRBCBA--JA503000
        search_padded = re.compile(b'.*'+search_value_padded_bytes+b'.*', flags=re.IGNORECASE|re.MULTILINE|re.DOTALL)
        operators = [
        ...
        for k1, k2, k3 in key_perms:
            for o1, o2, o3 in op_perms:
                ...
                candidate = [bytes(decoder[b] for b in e) for e in self._firmware_encrypted]
                decrypted = b''.join(candidate)
